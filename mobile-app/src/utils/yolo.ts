...
export const YOLO_ANCHORS = new Tensor(
  'float32',
  Float32Array.from([1.08, 1.19, 3.42, 4.41, 6.63, 11.38, 9.42, 5.11, 16.62, 10.52]),
  [5, 2],
)
...
export async function postprocess(outputTensor: Tensor) {
  ...
  const [preKeepBoxesArr, scoresArr] = await Promise.all([boxes.data, scores.data])
  ...
  const classesIndxArr = (await yolo.gather(classes, new Tensor('int32', keepIndx))
...
export function yolo_boxes_to_corners(boxXy: Tensor, boxWh: Tensor, boxTheta: Tensor) {
  ...
  const col1 = yolo.concat(
    [yolo.mul(yolo.div(height, negativeTwo), sinTheta), yolo.mul(yolo.div(height, two), cosTheta)],
    2,
  )
